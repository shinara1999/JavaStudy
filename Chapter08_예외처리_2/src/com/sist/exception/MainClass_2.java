 ...
 * 								  --- => 예외처리
 * 		   견고 => 프로그래머의 실수 , 사용자의 입력이 잘못된 경우
 * 		   --------------------------------------------
 * 		2. 예외처리
 * 			=> 정의 : 사전에 에러를 방지
 * 			=> 목적 : 비정상 정료를 방지하고 정상 종료를 할 수 있게 만든다.
 * 			---
 * 			---
 * 			---
 ...
 * 							문장 2
 * 							------	
 * 							문장 3 => 에러 발생 (수행하지 않고 catch로 이동) 
 * 							문장 4
 * 							문장 5
 * 							------ 무시
 * 						}catch(예외처리 클래스)
 * 						{
 * 							=> try 안에 있는 명령문에서 오류 발생시에 예외처리 / 복구
 * 						} => 여러개 사용 가능
 * 						finally ==> 생략 (필요한 경우에만)
 * 						{
 ...
 *  						문장 7
 *  					}
 *  					문장 8 ==> 종료 (얘가 나와야 정상 종료이다.)
 *  					
 *  					1 => 2 => 3 => 4 => 5 => 7 => 8 ==> try가 정상이면 catch는 수행하지 않는다.
 *  			
 *  					문장 4에서 에러 발생:
 *  					1 => 2 => 3 => 6 => 7 => 8
 *  
