 ...
 * 			}
 * 			인터페이스로 관리할 수 없다. ==> 
 * 			I i=new A() => i를 가지고 A가 가지고 있는 메소드 제어
 * 				=> 접근을 할 수 없는 메소드
 * 				aaa() , bbb() , ccc() , ddd()                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
 * 			I i=new B() => i를 가지고 B가 가지고 있는 메소드 제어
 * 			I i=new C() => i를 가지고 C가 가지고 있는 메소드 제어
 * 			==> A, B, C가 유형이 같은 경우 한개의 이름으로 제어
 * 				인터페이스의 목적 : 서로 다른 클래스를 묶어서 관리
 * 								------------- 관련
 * 								=> 관리 (소스코딩이 간결해진다.)
 * 								=> 인터페이스의 메소드 동일
 * 								예) 
...
{		// 구현(상속) ==> class(단일상속) , interface(다중상속)
	...
	{
		// display();
		I.display(); // interface의 static메소드는 => interface명으로만 접근 가능하다.
	}
}
