 ...
 * 									A(int c, int d) => A(int, int) ====> 중복! (오버로딩이 되지 않는다.)
 * 									=> 실제 메소드에도 적용
 * 									*** void도 리턴형이다.
 * 							기타
 ...
 * 				메소드명();
 * 			int 메소드명()
 * 			--- return값보다 데이터형이 크거나 같아야 한다.
 * 			{
 * 				return 정수값;
 * 					   -----
 * 			}
 * 				int a=메소드명() // 넘어오는 값을 받으려면 데이터형을 일치시켜야 한다.
 * 			int[] 메소드명()
 * 			{	
 * 				arr=new int[5];
 * 				return arr;
 ...
 * 					8
 * 					9		
 * 					return; => 항상 return이 걸리는 곳에서 끝난다.
 * 
 * 				}
 * 			
 * 				4 > 5 > 1 > 2 > 3 > 6 > 7 > 1 > 2 > 3 > 8 > 9
 * 				반복 제거 , 재사용 , 유지보수가 용이 (소스가 간결)
 * 								 -----------------------
 * 										ㅣ 구조적인 프로그램
 * 				객체지향 프로그램 => 메소드
 * 
