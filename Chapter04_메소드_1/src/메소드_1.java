 ...
 * 			= 다른 클래스와 연결 (메세지)
 * 			  객체지향 프로그램은 여러개의 클래스를 만들어서 연관관계를 설정
 * 			  => Scanner (사용자정의)
 * 				 scan.nextInt()
 * 					  --------- 입력된 정수 읽기
 * 				 System.out.println() => 화면 출력
 * 				 trim() , length()
 * 				 => 메소드는 한가지 기능만 수행 가능하게 만든다.
 * 					1) 입력
 * 					2) 요청 처리 => 기능에 따라서 세분화될수 있다. (여러개의 메소드 제작 가능)
 * 					3) 출력
 * 			= 반복 수행을 제거
 * 			  오라클 연결
 * 		 	  --------
 ...
 * 			*** 약속 사항
 * 				=> 소문자로 시작한다
 * 				=> 2개 단어가 있는 경우 두번째 단어의 첫자는 대문자 사용 or _
 * 				=> 데이터 저장 : set변수명()...
 * 				=> 데이터 읽기 : get변수명()...
 * 		3) 메소드 제작 시 고민
 * 		   ----------------------------------------------
...
public class 메소드_1 {
	static void a()
	{
		...
		return; // 생략 가능 => 값을 보내줄 때 한개만 사용 가능
	}
	// 시작점
	public static void main(String[] args) {
