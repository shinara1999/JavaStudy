 ...
 * 												  ---------- 확장이 불가능하다. 있는 그대로 사용해야 함(암기)
 * 													String , StringBuffer , Math , System , Wrapper , 예외 : Object
 * 			=> Object : 최상위 클래스 => 모든 클래스의 상위 클래스
 * 						(사용자 정의 , 라이브러리 => 모든 클래스 Object로부터 상속을 받는다.)
 * 						=toString() : 객체를 문자열화
 * 									  기본은 메모리 주소 출력 => 오버라이딩 (멤버변수 값 확인)
 * 						= clone() : 메모리 주소를 복제 => 새로운 메모리 생성
 * 						  게임 (아바타) => 디자인 패턴, 알고리즘
 * 										-----------------  스프링 전에
 * 						= finalize : 소멸자 (자동 호출) => 메모리 해제 시
 * 			=> System : 출력 , 메모리해제 , 프로그램 종료
 * 						= ***println() : 에러 출력 , 멤버변수 값 확인 , 전송 값 확인
 *						  윈도우 / 브라우저 => 자체 출력 = 디버깅
 *						= gc() : 가비지 컬렉션을 호출할 메모리를 비워준다.
 *							=> 멀티미디어 , 애니메이션
 *								==> 실시간 도로 cctv
 *						= exit() => 종료
 *						 	 ----	0(정상) , 비정상
 ...
 *					  => *** ceil() => 올림 => 총페이지 구하기
 *			=> StringBuffer : String을 보완 => 문자열 결합시 최적화 =>  *** append()
 *			=> String : 문자열 관리하는 클래스
 *
...
 							*** public boolean startsWith(String s)
					...
					3. 두개의 String에 값만을 비교해서 같으면 true, 다르면 false를 반환한다.(대소비교)
							...
							-------
								...
								=> 객체에는 heap에 저장되어 있는 메모리 주소를 가지고 있다.
															-------- 참조 변수
								   (주소 비교시에는 ==, 실제 저장된 값을 비쇼 equals)
								   *** public boolean equals(String s)
								   	   => 웹 : 로그인 , 아이디 찾기 , 아이디 중복
								   	   	  -- CRUD (게시판) => 회원가입 => 로그인 ...
								   	   	  -- 목록 (페이지 나누기) => 상세보기 (댓글)
								   	   	  -- 최신 방문 (쿠키) , 로그인상태 유지 (세션)
								   	   => 대소문자를 구분해서 비교
					...
					9. 지정한 문자로 문자열을 나눌수 있다.(배열로 반환)
							public String[] split(String regxp) => 정규식 이용
...
 
					...
					18. 문자와 문자를 결합해준다.
							public String concat(String s) ==> +와 같은 의미이다. 그래서 자주 사용하지 않는다.
							String s="A";
							String s1="B";
							s.concat(s1) => "AB"
							=
