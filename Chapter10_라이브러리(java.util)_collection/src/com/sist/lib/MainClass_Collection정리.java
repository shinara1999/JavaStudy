...
/*
 		컬렉션 프레임워크
 			=> 군집 (다수의 데이터)를 쉽게 관리할 수 있게 만든 라이브러리
 									...
 									--------- 파일 , 오라클
 											 ----   -----
 											  ㅣ       ㅣ select(읽기) , insert(추가) , update(수정)
 											  ㅣ	read() , write() , delete()
 											  파일 / 오라클
 											  -----------
 											  1. 읽기 => file은 한줄씩 => 구분자
 											  2. 오라클 => 원하는 데이터만 가지고올 수 있다.
 			
 			=> 동적 메모리 할당 (자동으로 크기 변경 => 크기를 지정하지 않는
 							 --------------- 가변형 (메모리 누수방지
 			...
 			=> ***컬렉션의 종류
 			   ...
 			   Set  : HashSet
 			     => 순서가 없다.
 			     => 중복을 허용하지 않는다.
 			        ** null값을 추가할 수 있다. (한 번만 가능, 두개 들어가면 null 중복)
 			     => 장르별 , 부서별 , 상품별 ...
 			     => List에서 중복 데이터를 제거할 때 사용
 			     => 주요메소드
 			        1) 추가 : add()
 			        2) 삭제 : remove(Object)
 			        3) 존재여부 : isEmpty()
 			        4) => 일반 for은 사용할 수 없다. => for-each 구문 사용
 			        5) 갯수 : size()
 		  ...
 		  프로그램 : 데이터 관리
 		  		   --------
 		  		     ...
 		  		     -------------
 		  		     	...
 		  		     	ㅣ => 이미 만들어진 클래스 (데이터형)
 		  		     	     -------------------------
 		  		     	     	ㅣ 라이브러리 => 지정하는 리턴형 / 매개변수의 데이터형을 변경
 		  		     	     			      => 프로그램 개발시마다 형변환을 해줘야 한다.
 		  		     	     			      => 필요시마다 데이터형을 변경
 		  		     	     			      => 보완 => 제네릭
