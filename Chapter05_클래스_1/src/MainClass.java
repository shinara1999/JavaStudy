 ...
 * 						static int a;
 * 					}
 * 			 지역변수 : 메소드 호출시마다 생성 => 메소드 안에서 사용 가능
 * 					class ClassName
 * 					{
 * 						void display()
 * 						{
 ...
 * 					=> 메모리에 저장 ===> 활용 ===> 메모리 해제
 * 				   *** 클래스는 한 개의 정보를 모아둔 곳 (영화 한 개에 대한 클래스)
 * 				   class Movie
 * 				   {
 * 						---
 * 						---
...
//	      for(int i=0;i<3;i++)
//	      {
//	         System.out.printf("%-5d%-5d%-5d%-7d%-7.2f\n",kor[i],eng[i],mat[i],kor[i]+eng[i]+mat[i],(kor[i]+eng[i]+mat[i])/3.0);
//	      }
	      ...
	      hong.eng=56;
	      hong.mat=34;
	      System.out.println(hong.name+" "+hong.kor+" "+hong.eng+" "+hong.mat+" "+(hong.kor+hong.eng+hong.mat));
	      //	  변수명 => 구분 (객체, 인스턴스)
	      //		   초기화
	      // 클래스(사용자 정의 데이터형)
	      // int   a  = 10
	      ...
	      shim.eng=56;
	      shim.mat=34;
	      System.out.println(shim.name+" "+shim.kor+" "+shim.eng+" "+shim.mat+" "+(shim.kor+shim.eng+shim.mat));
	      ...
	      park.eng=56;
	      park.mat=34;
	      System.out.println(park.name+" "+park.kor+" "+park.eng+" "+park.mat+" "+(park.kor+park.eng+park.mat));
