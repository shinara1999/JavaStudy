 ...
 * 			=> 머신러닝 / 딥러닝
 * 		 IT => 전진 => 앞만 보고 => 모르는 부분 (그 부분만 공부)
 * 		 =====> 실무 구현 (이론:면접)
 * 		자바
 * 		= 데이터형 => 변수 (2장)
 * 			정수 : byte , int , long
 * 				  ----		   ----
 * 				  ㅣ 파일입출력 (io) ㅣ 데이터분석 (빅데이터)
 * 			실수 : double
 * 			논리 : boolean
 * 			문자열 : String
 * 		= 처리 => 연산자/제어문 (3장, 4장)
 ...
 * 		  ------
 * 			멤버변수
 * 				= 인스턴스변수 (중심) ===> 메모리를 따로 저장
 * 				= static변수 (정적변수) ===> 메모리 공간을 한개만 사용
 * 				  ------------------
 ...
 * 						scan.next()		
 * 						s.replace()
 * 				= 정적메소드 (static) : 공유(한 개의 메소드로 모든 객체가 사용)
 * 					클래스명.메소드()
 * 					------
 * 					Math.random()
 * 					String.valueOf()
 ...
 * 					   매개변수 : 여러개 사용 => 사용자 요청값
 * 								3개 이상이면 => 클래스 , 배열
 * 								=> 회원가입 , 게시물글쓰기 , 영화정보 올리기
 * 			생성자
 * 				= 멤버변수 초기화 , 시작과 동시에 처리
 * 				= 메모리 할당시 호출
 * 				= 클래스는 무조건 생성자 1개이상이 필요하다.
 * 					=> 없는 경우에는 자동으로 1개 추가된다. (디폴트 생성자)
 * 				*** 자동 추가
 * 					1) import 자동 추가
 * 					   import java.lang.*
 ...
 * 					3) 모든 메소드는 return(메소드 종료점)을 필요로 한다.
 * 						=> void일 경우에는 자동으로 return 추가
 * 						void aaa()
 * 						{
 * 							// return;
 * 						}
 ...
 * 							--------------
 * 						}	
 * 					5) 자바에서 지원하는 모든 클래스는 예외처리를 해야한다.
